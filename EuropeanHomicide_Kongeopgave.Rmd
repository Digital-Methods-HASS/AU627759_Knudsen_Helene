...
author: "Adela Sobotkova, Helene Kjær Knudsen"
...
Answer: The amount of homicides falls the further you get into resent years, and at the same time the lengths of the kings reigns get longer.


The bellow chunks stops it from knitting, I don't know why
...
```{r kongedataset behandling}
kongedata <- read_csv2("data/Data kongerækken Simpel Rigtige Rigtig.csv")
kongedata <- kongedata %>% 
  mutate(mid_year = Reign_End-(Reign_End-Reign_start)/2)
```

```{r}
kongedata %>% 
  ggplot(aes(x=mid_year, y=Years_Rulled))+
  geom_smooth()
```
...
Need to use colour="Country", as can be seen the name has changed.
...
If you were to say that the amount of murders per 100 thousand people matters for how civilized we are as human beings. Then from our visualization it shows that the amount of murders have fallen quite drastically since where the data set starts in the 1500 hundreds. 
Of course that does not mean that the amount of murders are at 0, but we as humans in the 5 areas have gone down. We can also see that all the 5 areas are down to very few murders. Humans will probably never stop murdering each other sadly, but does that mean that we are not civilized today? 
I would argue that humans have always been civilized. Murderess do not make it so all human beings are uncivilized. And that the amount of murders are going down as we can see in the visualizations that they have gone down can say we are being even more civilized today than we have ever been.