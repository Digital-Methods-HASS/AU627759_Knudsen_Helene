...
The bottom 3 chunks stops it from knitting, I don't know why
Here we remove the NA values
```{r}
#kongedata_na <- na.omit(kongedata)
```


```{r}
#mid_year <- kongedata_na([3]-([3]-[2])/2) 
```
...
Have not been able to make the last task.
